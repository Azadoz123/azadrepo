    ...
    public class Customer_Generic_Repository_Tests: ModuleDomainTestBase
    {
        ...
        public async Task Should_Get_Customer()
        {
            await Assert.ThrowsAsync<EntityNotFoundException>(
                async () => await _repository.GetAsync(Guid.Empty));  //Kayıt bulunmazsa

            await Assert.ThrowsAsync<InvalidOperationException>(
                async () => await _repository.GetAsync(c => c.Id != Guid.Empty)); //Birden çok kayıt dönerse
        ...
        public async Task Should_Find_Customer()
        {
            ...
            await Assert.ThrowsAsync<InvalidOperationException>(
                async () => await _repository.FindAsync(c => c.Id != Guid.Empty)); //Birden çok kayıt dönerse

            var customerId = await WithUnitOfWorkAsync<Guid>(
                async () => ( await _repository.FirstOrDefaultAsync()).Id);
            var custmer = await _repository.FindAsync(customerId); 
            custmer.ShouldNotBeNull();
        }
